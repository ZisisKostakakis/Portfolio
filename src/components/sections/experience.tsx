...
function ExperienceCard({ exp }: { exp: (typeof experiences)[number] }) {
  return (
    <div className="relative p-6 rounded-xl bg-primary-navy-light border border-primary-gray-dark hover:border-primary-violet/30 hover:shadow-glow-sm transition-all duration-200 before:absolute before:left-0 before:top-6 before:bottom-6 before:w-0.5 before:rounded-full before:bg-gradient-to-b before:from-primary-violet/60 before:to-transparent">
      ...
      <p className="text-primary-violet font-medium mb-3">{exp.position}</p>
      ...
      <ul className="space-y-2 mb-4">
        {exp.description.map((item, i) => (
          <li key={i} className="text-sm text-primary-slate flex items-start gap-2">
            <span className="text-primary-violet mt-1.5 flex-shrink-0">&#8226;</span>
            {item}
          </li>
        ))}
      </ul>
    ...
    <section id="experience" className="py-24 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Permanent column */}
          <div>
            <p className="font-mono text-xs font-semibold tracking-widest uppercase text-primary-violet/70 mb-6">
              Permanent
            </p>
